...
export interface DatabaseService {
  ...
  getItem<T>(tableName: string, key: Record<string, any>): Promise<T | null>;
  ...
  batchWrite(
    tableName: string,
    items: { put?: DynamoDBItem[]; delete?: Record<string, any>[] }
  ): Promise<void>;

  // Utility operations
  itemExists(tableName: string, key: Record<string, any>): Promise<boolean>;
...
class DynamoDBService implements DatabaseService {
  ...
  async itemExists(tableName: string, key: Record<string, any>): Promise<boolean> {
    try {
      const result = await dynamodb.send(
        new GetCommand({
          TableName: tableName,
