...
interface EventWithQueryParams {
...
export const parseQueryParams = (event: EventWithQueryParams) => {
...
export const parsePathParams = (event: EventWithPathParams) => {
...
interface EventWithBody {
  body: string | null;
}

export const parseRequestBody = <T>(event: EventWithBody): T => {
  if (!event.body) {
...
interface EventWithTenant {
  tenant?: unknown;
}

export const extractUserFromEvent = (event: EventWithUser) => {
  if (!event.user) {
...
export const extractTenantFromEvent = (event: EventWithTenant) => {
  if (!event.tenant) {
...
export const checkTenantLimits = (tenant: TenantWithLimits, resource: string, increment = 1): boolean => {
