...
# S3 Bucket Object for Lambda deployment (for large functions > 70MB)
resource "aws_s3_object" "main_api_zip" {
  bucket = data.aws_s3_bucket.static_assets.bucket
  key    = "lambda-deployments/main-api-${data.archive_file.main_api_zip.output_base64sha256}.zip"
  source = data.archive_file.main_api_zip.output_path
  etag   = data.archive_file.main_api_zip.output_md5
  
  tags = merge(local.common_tags, {
    Name    = "${local.name_prefix}-main-api-deployment"
    Purpose = "Lambda deployment package"
  })
}

# Main API Lambda Function (using S3 bucket for large deployment)
resource "aws_lambda_function" "main_api" {
  s3_bucket        = aws_s3_object.main_api_zip.bucket
  s3_key           = aws_s3_object.main_api_zip.key
