  ...
  Amplify.configure({
    Auth: {
      Cognito: {
        ...
        loginWith: {
          oauth: {
            domain: import.meta.env.VITE_AUTH_DOMAIN || `ai-nexus-workbench-${getEnvironment()}.auth.${config.region}.amazoncognito.com`,
            scopes: ['openid', 'profile', 'email'],
            redirectSignIn: ['http://localhost:8080/', import.meta.env.VITE_APP_BASE_URL || 'https://dev.diatonic.ai/'],
            redirectSignOut: ['http://localhost:8080/', import.meta.env.VITE_APP_BASE_URL || 'https://dev.diatonic.ai/'],
            responseType: 'code'
          },
          username: true,
          email: true,
