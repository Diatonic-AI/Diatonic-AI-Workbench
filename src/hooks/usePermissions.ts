...
export interface UsePermissionsReturn {
  ...
  hasRole: (role: UserRole) => boolean; // Alias for isRoleAtLeast
...
export const usePermissions = (): UsePermissionsReturn => {
  ...
  // hasRole is an alias for isRoleAtLeast for backward compatibility
  ...
  const isOrgUser = useMemo(() => role === 'pro' || role === 'extreme' || role === 'enterprise', [role]);
  const isDevelopmentUser = useMemo(() => role === 'internal_dev', [role]);
  const isTestingUser = useMemo(() => role.startsWith('internal_'), [role]);
  ...
  return {
    ...
    // Role checking functions
    isRole,
    hasRole, // Added for backward compatibility
    isRoleAtLeast,
