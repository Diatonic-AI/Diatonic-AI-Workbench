/**
 * Local Authentication Middleware for Express Server
 * Simplified version of the unified auth middleware for use in Express server
 */
...
const jwks = jwksClient({
  jwksUri: `https://cognito-idp.${process.env.AWS_REGION || 'us-east-2'}.amazonaws.com/${process.env.COGNITO_USER_POOL_ID || 'us-east-2_xkNeOGMu1'}/.well-known/jwks.json`,
  cache: true,
...
function verifyToken(token: string): Promise<JWTClaims> {
  return new Promise((resolve, reject) => {
    jwt.verify(token, getKey, {
      issuer: `https://cognito-idp.${process.env.AWS_REGION || 'us-east-2'}.amazonaws.com/${process.env.COGNITO_USER_POOL_ID || 'us-east-2_xkNeOGMu1'}`,
      audience: process.env.COGNITO_CLIENT_ID || '4ldimauhip6pq3han3ot5u9qmv',
      algorithms: ['RS256'],
    }, (err, decoded) => {
      if (err) {
        return reject(err);
...
export async function localAuthMiddleware(
  ...
  try {
    ...
    const userContext: UserContext = {
      ...
      permissions: generateFallbackPermissions(role, plan, features),
      subscriptionLimits: {},
      quotaUsage: {},
      organizations: [],
      cognitoGroups,
